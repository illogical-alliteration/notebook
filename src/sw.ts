...
const swContext = self as unknown as ServiceWorkerGlobalScope;

const CACHE_NAME = `static-cache-${version}`;

// install handler
swContext.addEventListener('install', (event: ExtendableEvent) => {
  event.waitUntil(
    (async () => {
...
// fetch handler
swContext.addEventListener('fetch', (event: FetchEvent) => {
